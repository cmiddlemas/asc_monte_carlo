...
cargo run --release -- --no-rayon -d 3 -n 200 -s 16 --cell-list --pressure inf --pcell 0.005 --moves 4000 --sweeps 50000 --adjust --gap-threshold 1e-6 --logfiles $TEST_DIR/$RUN_NAME/initial_log -o $TEST_DIR/$RUN_NAME/initial_config 2>$TEST_DIR/$RUN_NAME/initial.stderr >$TEST_DIR/$RUN_NAME/initial.stdout
