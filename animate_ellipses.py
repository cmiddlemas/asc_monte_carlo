...
def update(frame):
    ...
    try:
        ...
        for i in range(0,len(config)):
            ellipse = Ellipse((config[i,0], config[i,1]), 2.0*config[i,3], 2.0*config[i,4], angle=180.0*config[i,2]/np.pi)
...
if __name__ == "__main__":
    ...
    animation.save(sys.argv[1], writer='imagemagick')
